// Inside an SSD: NAND, Pages, Blocks and the Flash Translation Layer - slide 9: garbage_collect: move, then erase
// Build: make 09_garbage_collect
#include <iomanip>
#include <iostream>
...
void write_no_gc(Ftl& f, int lba) {
    ++f.host_writes;
    if (f.map[lba] >= 0) state_of(f, f.map[lba]) = Page::Stale;
    program(f, lba);
}
...
int main() {
    ...
    for (int lba : {0, 1, 2, 5}) write_no_gc(f, lba);         // four overwrites: block 0 gets 3 stale pages, block 1 one
    std::cout << "before: block 0 holds the most stale pages, so it is the victim\n";
    print_blocks(f);
    std::cout << "  victim(f) = " << victim(f) << ", erased blocks left = " << erased_blocks(f) << "\n";
    ...
    std::cout << "\nafter one garbage_collect(): " << (collected ? "block erased" : "nothing to do") << "\n";
    print_blocks(f);
    for (int lba = 0; lba < LBAS; ++lba)
        if (before[lba] != f.map[lba])
            std::cout << "  moved LBA " << lba << ": page " << before[lba] << " to page " << f.map[lba] << "\n";
    std::cout << "  extra NAND writes the host never asked for: " << f.nand_writes - nand_before << "\n";
    report("after the collection", f);

    Ftl clean = make_ftl();
    for (int lba = 0; lba < 4; ++lba) write(clean, lba);
    bool again = garbage_collect(clean);
    std::cout << "\na drive with no stale page: garbage_collect() returns " << std::boolalpha << again << "\n";
